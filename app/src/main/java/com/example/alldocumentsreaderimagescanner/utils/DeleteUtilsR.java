...
public class DeleteUtilsR {
    ...
    public void deleteFile(String path, DeleteCallBack deleteCallBack) {
        this.deleteCallBack = deleteCallBack;
        ArrayList<Uri> arrayList = new ArrayList<>(Arrays.asList(ContentUri(activity, path, "File")));
        if (Build.VERSION.SDK_INT >= 30) {
            delete1Array(arrayList);
        } else {
            if (delete0Array(new ArrayList<>(Arrays.asList(path)))) {
    ...
    private Uri ContentUri(Context context, String path, String type) {
        ...
        switch (type) {
            ...
            case "File":
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    mediaStore =MediaStore.Files.getContentUri(MediaStore.VOLUME_EXTERNAL);
            } else {
                    mediaStore = MediaStore.Files.getContentUri("external");
                    contentUri = "content://media/external/video/media";
            }
                break;
        }
        ...
        if (cursor != null && cursor.moveToFirst()) {
            return Uri.withAppendedPath(mediaStore, "" + cursor.getInt(cursor.getColumnIndex("_id")));
        } else if (!file.exists()) {
            return null;
        } else {
