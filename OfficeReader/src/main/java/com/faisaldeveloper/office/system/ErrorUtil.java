...
import com.faisaldeveloper.office.fc.OldFileFormatException;
...
public class ErrorUtil {
    ...
    private void processThrowable(final Throwable ex, final boolean isReaderFile) {
        ...
        if (control.isAutoTest()) {
            ...
            control.getActivity().getWindow().getDecorView().post(() -> {
                try {
                    String err = "";
                    ...
                    if (error.contains("SD")) {
                        ...
                        Toast.makeText(activity, "Maybe file not proper show", Toast.LENGTH_SHORT).show();
