...
package com.faisaldeveloper.office.common.bulletnumber;
...
public class ListKit
{
	...
	{
		String tempstr;
		if (y.charAt(1) == '0')// 判断是否小于十
		{
			tempstr = zr3(y);
		}
		else if (y.charAt(1) == '1')// 判断是否在十到二十之间
		{
			tempstr = zr2(y);
		}
		else
		{
			if (y.charAt(2) == '0')// 判断是否为大于二十小于一百的能被十整除的数（为了去掉尾空格）
			{
				tempstr = zr1(y);
			}
			else
	...
	{
		String tempstr;
		if (y.charAt(0) == '0')// 判断是否小于一百
		{
			tempstr = w2(y);
		}
		else
		{
			if (y.startsWith("00", 1))// 判断是否能被一百整除
			{
